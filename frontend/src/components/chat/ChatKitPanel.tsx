...
export function ChatKitPanel({
  ...
  const chatkit = useChatKit({
    ...
    onClientTool: async (invocation: {
      ...
      if (invocation.name === "create_task") {
        ...
        await onTaskAction?.({
          type: "create",
          taskData: invocation.params,
        });
        return { success: true };
      }

      if (invocation.name === "complete_task") {
        const taskId = String(invocation.params.task_id ?? "");
        const actionId = `complete-${taskId}`;
        if (processedActions.current.has(actionId)) {
          return { success: true };
        }
        processedActions.current.add(actionId);
      ...
      if (invocation.name === "delete_task") {
        const taskId = String(invocation.params.task_id ?? "");
        const actionId = `delete-${taskId}`;
        if (processedActions.current.has(actionId)) {
          return { success: true };
        }
        processedActions.current.add(actionId);
  ...
  return (
    <div className="relative flex h-[85vh] w-full rounded-2xl flex-col overflow-hidden bg-white shadow-lg transition-colors dark:bg-slate-900 border border-slate-200 dark:border-slate-700">
      <ChatKit
